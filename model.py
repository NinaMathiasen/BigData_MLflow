...
if __name__ == "__main__":
    ...
    param_n_estimators = float(sys.argv[1]) if len(sys.argv) > 1 else 100
    param_max_depth = float(sys.argv[2]) if len(sys.argv) > 2 else 1000
    param_min_samples_split = float(sys.argv[3]) if len(sys.argv) > 3 else 2
    param_min_samples_leaf = float(sys.argv[4]) if len(sys.argv) > 4 else 1
    ...
    with mlflow.start_run():
        ...
        if tracking_url_type_store != "file":
            ...
            mlflow.sklearn.log_model(model_pipeline, "model", registered_model_name="RandomForestModel")
        else:
